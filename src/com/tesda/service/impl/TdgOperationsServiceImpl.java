...
public class TdgOperationsServiceImpl extends TdgBaseServiceImpl implements TdgOperationsService{
	...
	@Autowired
	TdgCustomUpdateDao tdgCustomUpdateDao;
	...
	public String generateTestData(TestDataGenerateDTO testDataGenenarteDTO){
		...
		try {
			managerentity = openEntityManager();
			if (managerentity != null) {
				if("INSERT".equalsIgnoreCase(testDataGenenarteDTO.getPopulationType())){
				strResult = tdgCustomDao.generateTestData(testDataGenenarteDTO, managerentity);
				}else if("UPDATE".equalsIgnoreCase(testDataGenenarteDTO.getPopulationType())){ 
				strResult = tdgCustomUpdateDao.generateTestData(testDataGenenarteDTO, managerentity);
				}else if("DELETE".equalsIgnoreCase(testDataGenenarteDTO.getPopulationType())){
					strResult = tdgCustomUpdateDao.generateTestData(testDataGenenarteDTO, managerentity);
				}
			}
		} finally {
	...
	@Override
	public Map<String, List<String>> retrieveManualDictionaryValues(String strDictionaryName,String schemaname){
		...
		try {
			managerentity = openEntityManager();
			if (managerentity != null) {
				mapResult = tdgOperationsDao.retrieveManualDictionaryValues(strDictionaryName,0,
						managerentity,schemaname);
			}
		} finally {
			if (managerentity != null) {
				closeEntityManager(managerentity);
	...
	@Override
	public List<TdgDynamicPageContentDTO> getSchemaDetailsForDynamicPage(String dictionaryName){
		String strMethodName = "  [ getAllSchemaDetailsForDynamicPage() ] ";
		logger.info(strClassName + strMethodName
				+ " inside getAllSchemaDetailsForDynamicPage method");
		List<TdgDynamicPageContentDTO> listResult = new ArrayList<TdgDynamicPageContentDTO>();
		...
		try {
			managerentity = openEntityManager();
			if (managerentity != null) {
				TdgSchemaDO lstTdgSchemaDO = tdgOperationsDao
						.fetchSchemaDetailsByName(dictionaryName, managerentity);
				//if (listResult != null) {
					//for (TdgSchemaDO tdgSchemaDO : lstTdgSchemaDO) {
						List<String> lstColumnsNames = null;
						if (!StringUtils.isEmpty(lstTdgSchemaDO.getManualdictionary())) {
							lstColumnsNames = tdgOperationsDao.retrieveManualDictionaryColumns(
									lstTdgSchemaDO.getManualdictionary(), managerentity);
						}
						listResult.add(tdgOperationsMapper
								.convertTdgSchemaDOToTdgDynamicPageContentDTO(lstTdgSchemaDO,
										lstColumnsNames));
					//}
				//}
			}
