...
public class TdgOperationsMapperImp implements TdgOperationsMapper{
	...
	public TdgSchemaDTO convertTdgSchemaDOToTdgSchemaDTO(TdgSchemaDO tdgSchemaDO){
		...
		if (tdgSchemaDO != null) {
			...
			tdgSchemaDTO.setDataconnections(tdgSchemaDO.getDataconnections());
			tdgSchemaDTO.setColumnwithheader(tdgSchemaDO.getColumnwithheader());
			...
			for (TdgGuiDetailsDO tdgGuiDetailsDO : setTdgGuiDetailsDO) {
				...
				tdgGuiDetailsDTO.setColumnCondition(tdgGuiDetailsDO.getColumnCondition());
	...
	public TdgSchemaDO convertTdgSchemaDTOToTdgSchemaDO(TdgSchemaDTO tdgSchemaDTO){
		...
		Set<TdgGuiDetailsDO> setTdgGuiDetailsDO = new LinkedHashSet<TdgGuiDetailsDO>();
		if (tdgSchemaDTO != null) {
			...
			tdgSchemaDO.setDataconnections(tdgSchemaDTO.getDataconnections());
			tdgSchemaDO.setColumnwithheader(tdgSchemaDTO.getColumnwithheader());
			...
			for (TdgGuiDetailsDTO tdgGuiDetailsDTO : setTdgGuiDetailsDTO) {
				...
				tdgGuiDetailsDO.setColumnCondition(tdgGuiDetailsDTO.getColumnCondition());
	...
	public List<TdgDynamicPageContentDTO> convertTdgSchemaDOToTdgDynamicPageContentDTO(
		...
		for (TdgSchemaDO tdgSchemaDO : listTdgSchemaDO) {
			...
			if (tdgSchemaDO != null) {
				...
				tdgDynamicContentDTO.setColumnwithheader(tdgSchemaDO.getColumnwithheader());
				if(tdgSchemaDO.getUrl() != null && tdgSchemaDO.getUrl().contains("#")){
					StringBuffer strBuffer = new StringBuffer("");
					String args[] = tdgSchemaDO.getUrl().split("#");
					for(int i=0;i<args.length;i++){
						if (args[i] != null
							...
							strBuffer.append(TdgCentralConstant.DB_TYPE_ORACLE).append(" - ");
						} else if (args[i] != null
								&& args[i].toUpperCase()
										.contains(TdgCentralConstant.DB_TYPE_MYSQL.toUpperCase())) {
							//tdgDynamicContentDTO.setDbType(TdgCentralConstant.DB_TYPE_MYSQL);
							strBuffer.append(TdgCentralConstant.DB_TYPE_MYSQL).append(" - ");
						} else if (args[i] != null
								&& args[i].toUpperCase()
										.contains(TdgCentralConstant.DB_TYPE_DB2.toUpperCase())) {
							strBuffer.append(TdgCentralConstant.DB_TYPE_DB2).append(" - ");
							//tdgDynamicContentDTO.setDbType(TdgCentralConstant.DB_TYPE_DB2);
						}else if (args[i] != null
								&& args[i].toUpperCase()
										.contains(TdgCentralConstant.DB_TYPE_SQLSERVER.toUpperCase())) {
							strBuffer.append(TdgCentralConstant.DB_TYPE_SQLSERVER).append(" - ");
							//tdgDynamicContentDTO.setDbType(TdgCentralConstant.DB_TYPE_SQLSERVER);
						} else if (args[i] != null
								&& args[i].toUpperCase()
										.contains(TdgCentralConstant.DB_TYPE_POSTGRES.toUpperCase())) {
							tdgDynamicContentDTO.setDbType(TdgCentralConstant.DB_TYPE_POSTGRES);
						}else if (args[i] != null
								&& args[i].toUpperCase()
										.contains(TdgCentralConstant.DB_TYPE_DERBY.toUpperCase())) {
							tdgDynamicContentDTO.setDbType(TdgCentralConstant.DB_TYPE_DERBY);
						}
					}
					
					tdgDynamicContentDTO.setDbType(strBuffer.substring(0,strBuffer.toString().lastIndexOf(" - ")));
					}else{
						//StringBuffer strBuffer = new StringBuffer("");
				...
				for (TdgGuiDetailsDO tdgGuiDetailsDO : setTdgGuiDetailsDO) {
					...
					tdgDynamicGuiDTO.setColumnCondition(tdgGuiDetailsDO.getColumnCondition());
	...
	public TdgDynamicPageContentDTO convertTdgSchemaDOToTdgDynamicPageContentDTO(
		...
		TdgDynamicPageContentDTO tdgDynamicContentDTO = new TdgDynamicPageContentDTO();
		List<TdgDynamicGuiDTO> setTdgGuiDetailsDTO = new LinkedList<TdgDynamicGuiDTO>();
		List<TdgDynamicGuiDTO> setForDependsTdgGuiDetailsDTO = new LinkedList<TdgDynamicGuiDTO>();
		if (tdgSchemaDO != null) {
			tdgDynamicContentDTO.setPassword(tdgSchemaDO.getPassword());
			tdgDynamicContentDTO.setName(tdgSchemaDO.getUsername());
			tdgDynamicContentDTO.setUrl(tdgSchemaDO.getUrl());
			...
			for (TdgGuiDetailsDO tdgGuiDetailsDO : setTdgGuiDetailsDO) {
				TdgDynamicGuiDTO tdgDynamicGuiDTO = new TdgDynamicGuiDTO();
				tdgDynamicGuiDTO.setColumnId(tdgGuiDetailsDO.getReqguiid());
				tdgDynamicGuiDTO.setColumnLabel(tdgGuiDetailsDO.getColumnLabel());
				tdgDynamicGuiDTO.setColumnName(tdgGuiDetailsDO.getColumnname());
				tdgDynamicGuiDTO.setColumnType(tdgGuiDetailsDO.getColumnType());
				tdgDynamicGuiDTO.setColumnValues(tdgGuiDetailsDO.getColumnValues());
			...
			setTdgGuiDetailsDTO.addAll(setForDependsTdgGuiDetailsDTO);
			//sorting gui columns
			Collections.sort(setTdgGuiDetailsDTO, new Comparator(){
	            @Override
	            public int compare(Object softDrinkOne, Object softDrinkTwo) {
	                //use instanceof to verify the references are indeed of the type in question
	                return (((TdgDynamicGuiDTO)softDrinkOne).getColumnId()
	                        .compareTo(((TdgDynamicGuiDTO)softDrinkTwo).getColumnId()));
	            }
	        }); 
			//end
			tdgDynamicContentDTO.setListDynamicPojo(setTdgGuiDetailsDTO);
