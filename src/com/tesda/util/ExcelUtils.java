...
public class ExcelUtils extends TDGBaseUtil{
	...
	public static void doGenerateExcelXLSFile(List<Object[]> lstInputParams,
			String strFileName, int iTotalColumns,Map<Integer,String> mapPositions,List<String> lstRequiredColumnsSequence,boolean isAddedFlag, Map<String, String> mapColumnsOfTab) {
			...
			for (int irownum = 1; irownum < lstInputParams.size(); irownum++) {
				...
				for (int irownum = 0; irownum < lstInputParams.size(); irownum++) {
					...
					for (int iColumnLength = 0; iColumnLength < iTotalColumns; iColumnLength++) {
						...
						if(StringUtils.isNotEmpty(mapColumnsOfTab.get(objHeaders[iColumnLength])) && !"null".equalsIgnoreCase(mapColumnsOfTab.get(objHeaders[iColumnLength])) )
						cell.setCellValue(String.format("%-"+Integer.parseInt(mapColumnsOfTab.get(objHeaders[iColumnLength]))+ "s",String.valueOf(objArr[iColumnLength])));
						else
							cell.setCellValue(String.valueOf(objArr[iColumnLength]));
					}
				}
