...
public interface TdgOperationsDao{
	...
	String saveTablesDetails(Map<String, List<String>> mapResult,
			EntityManager managerentity);

	List<TdgSchemaDO> fetchSchemaDetailsToDownload(String reqScheamId, EntityManager managerEntity);

	Map<String, List<String>> retrieveManualDictionaryValues(String strTabName, int requiredCount,
			EntityManager entityManager, String schemaName);

	void doDumpTableAliasValues(Map<String, List<String>> mapExcelValues, String userid,
			EntityManager entityManager);

	TdgSchemaDO fetchSchemaDetailsByName(String dictionaryName, EntityManager managerentity);
}
