...
public class TDMAdminDAOImpl implements TDMAdminDAO{
	...
	@Override
	public String dropManualDictionaryDetails(List<String> strTabNames, EntityManager managerEntity){
		String strResponse = TdgCentralConstant.FAILED_MESSAGE;
		StringBuffer strCreateQuery = new StringBuffer(" DROP TABLE ");
		if (strTabNames != null && !strTabNames.isEmpty()) {
			StringBuffer buffer = new StringBuffer();
			for(String str:strTabNames){
				if(buffer.length()>0)
					buffer.append(",");
				buffer.append(str);
			}
				
			strCreateQuery.append(buffer.toString());
