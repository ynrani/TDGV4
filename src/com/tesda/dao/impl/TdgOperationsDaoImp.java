...
public class TdgOperationsDaoImp implements TdgOperationsDao{
		...
		try {
			...
			if (listTdgSchemaDao != null && !listTdgSchemaDao.isEmpty()) {
				logger.info(strClassName
						+ strMethodName
						+ " upload dictionary details is already exist. So going to remove the schema details");
						...
						if (tdgGuiDetailsDO.getColumnValues() != null
							...
							if (template != null) {
								try {
									...
									logger.debug("not able to connect the target data base ==> "
											+ se.getMessage());
									finallistDOs.clear();
									break;
								}
	...
	@Override
	public List<TdgSchemaDO> fetchSchemaDetailsToDownload(String reqScheamId,EntityManager managerEntity){
		String strMethodName = " [ fetchSchemaDetailsToDownload() ] ";
		logger.info(strClassName + strMethodName + " inside fetchSchemaDetailsToDownload method");
		List<TdgSchemaDO> listTdgSchemaDao = new ArrayList<TdgSchemaDO>();
		JdbcTemplate template = null;
		try {
			if (managerEntity != null) {
				//List<TdgSchemaDO> tempTdgSchemaDao = new ArrayList<TdgSchemaDO>();
				listTdgSchemaDao.addAll(managerEntity.createNamedQuery("TdgSchemaDO.findBySchemaId",
						TdgSchemaDO.class).setParameter("reqschemaid", Long.parseLong(reqScheamId)).getResultList());
				/*for (TdgSchemaDO tdgSchemaDO : tempTdgSchemaDao) {
					template = getTemplate(tdgSchemaDO.getUrl(), tdgSchemaDO.getUsername(),
							tdgSchemaDO.getPassword());
					TdgSchemaDO tempTdgSchemaDO = new TdgSchemaDO();
		...
		logger.info(strClassName + strMethodName + " return from fetchSchemaDetailsToDownload method");
		return listTdgSchemaDao;
	}
	...
	public String saveDashBoardDetails(TdgRequestListDO tdgRequestListDO,
		...
		try {
			if (managerEntity != null) {
				...
				tdgRequestListDO.setGeneratedate(cal.getTime());
				...
				doSaveDashBoardValues(no + 1, TdgCentralConstant.TDG_REQUEST_GEN_LIST,
						mapConditions,tdgRequestListDO.getRequestCount(), managerEntity);
				}
	...
	private void doSaveDashBoardValues(final long requestId, String strTabName,
			Map<String, List<String>> mapExcelValues,long lRequestedCount, EntityManager entityManager){
	...
	public String saveManualDictionaryDetails(String strTabName,
		...
		if (mapExcelValues != null && !mapExcelValues.isEmpty()) {
			...
			int iSize = mapExcelValues.size();
			for (Map.Entry<String, List<String>> entryKeys : mapExcelValues.entrySet()) {
				if(StringUtils.isNotEmpty(entryKeys.getKey())){
			...
			if (iCount != 1 && reqSchemaId != null && !reqSchemaId.isEmpty()) {
				strCreateQuery.append(')');
				managerEntity.getTransaction().begin();
		...
		}else if(StringUtils.isNotEmpty(tabName)){
			managerEntity.getTransaction().begin();
			TdgSchemaDO tdgSchemaDO = (TdgSchemaDO) managerEntity
					.createNamedQuery("TdgSchemaDO.findBySchemaId")
					.setParameter("reqschemaid", Long.parseLong(reqSchemaId)).getSingleResult();
	...
	@Override
	public Map<String, List<String>> retrieveManualDictionaryValues(final String strTabName,final int requiredCount, 
			EntityManager entityManager,String schemaName){
		...
		List<TdgTableAliasDO> aliasDo = null;
		if(StringUtils.isNotEmpty(schemaName)){
			aliasDo = entityManager.createNamedQuery("TdgTableAliasDO.findBySchemaName",TdgTableAliasDO.class).setParameter("schemaname", schemaName).getResultList();
		}
		final List<TdgTableAliasDO> aliasFinalDo =aliasDo;
		if (!StringUtils.isEmpty(strTabName) && entityManager != null) {
			entityManager.unwrap(Session.class).doWork(new Work(){
				...
				public void execute(Connection connection) throws SQLException{
					...
					if(requiredCount > 0)
					preparedStatement.setFetchSize(requiredCount);
					ResultSet resultSet = preparedStatement.executeQuery();
					ResultSetMetaData resultSetMetaData = resultSet.getMetaData();
					List<List<String>> listResult = new ArrayList<List<String>>();
					
					Map<String,List<String>> mapDumpValueforDashValues = new HashMap<String,List<String>>();
					for (int i = 1; i <= resultSetMetaData.getColumnCount(); i++) {
						List<String> listColumnValues = new LinkedList<String>();
						listResult.add(listColumnValues);
						boolean check = false;
						if(aliasFinalDo !=null){
							for(TdgTableAliasDO dos : aliasFinalDo){
								if(resultSetMetaData.getColumnLabel(i).toUpperCase().contains(dos.getAliasname().toUpperCase()+TdgCentralConstant.TDG_DOT)){
									mapDumpValueforDashValues.put(resultSetMetaData.getColumnLabel(i).replaceAll(dos.getAliasname().toUpperCase()+TdgCentralConstant.TDG_DOT, dos.getTabname().toUpperCase()+TdgCentralConstant.TDG_DOT), listColumnValues);
									check = true;
									break;
								}
							}
						...
						if(!check)
						mapDumpValueforDashValues.put(resultSetMetaData.getColumnLabel(i), listColumnValues);
					}
					while (resultSet.next()) {
						for (int k = 0; k < mapDumpValueforDashValues.size(); k++) {
							List<String> listValues = listResult.get(k);
							listValues.add(listValues.size(),resultSet.getString(k + 1));
						}
					}
					
					//final code for '#' seperated code 
					for(Entry<String, List<String>> strMapKey  : mapDumpValueforDashValues.entrySet()){
						if(strMapKey.getKey().contains("#")){
							String[] strSplits = strMapKey.getKey().split("#");
							mapResult.put(strSplits[0], strMapKey.getValue());
							mapResult.put(strSplits[1], strMapKey.getValue());
						}else{
							mapResult.put(strMapKey.getKey(), strMapKey.getValue());
						}
					}
				}
			});
			...
			entityManager.unwrap(Session.class).doWork(new Work(){
				...
				public void execute(Connection connection) throws SQLException{
					/*PreparedStatement preparedStatement = connection
							.prepareStatement("SELECT *FROM " + strTabName);*/
					PreparedStatement preparedStatement = connection
							.prepareStatement("select COLUMN_NAME from USER_TAB_COLUMNS where TABLE_NAME='" + strTabName+"'");
					ResultSet resultSet = preparedStatement.executeQuery();
					/*ResultSetMetaData resultSetMetaData = resultSet.getMetaData();
					for (int i = 1; i <= resultSetMetaData.getColumnCount(); i++) {
					...
					while(resultSet.next()){
						listColumnValues.add(resultSet.getString("COLUMN_NAME"));
					}
				}
			});
			logger.info(strClassName + strMethodName
	...
	@Override
	public String saveTablesDetails( Map<String, List<String>> mapExcelValues,
			EntityManager managerentity){
		String strMethodName = " [ saveTablesDetails() ] ";
		String strResponse = TdgCentralConstant.FAILED_MESSAGE;
		logger.info(strClassName + strMethodName + " inside saveTablesDetails method");
		StringBuffer strCreateQuery = null;
		List<String> lstFinalQueries = new ArrayList<String>();
		if (mapExcelValues != null && !mapExcelValues.isEmpty()) {
			/* Going to drop table if already exist in system */
			for (Map.Entry<String, List<String>> mapEntry : mapExcelValues.entrySet()) {
				int iCount = 1;
				int iSize = mapEntry.getValue().size();
				strCreateQuery = new StringBuffer(" CREATE TABLE ");
				strCreateQuery.append(mapEntry.getKey()).append('(');
				for (String strVal : mapEntry.getValue()) {
					if (strVal.contains("#")) {
						String[] splits = strVal.split("#");
						strCreateQuery.append(splits[0]).append(' ')
								.append(TdgCentralConstant.ORACLE_VARCHAR2).append("(")
								.append(splits[1]).append(")");
					} else {
						strCreateQuery.append(strVal).append(' ').append(TdgCentralConstant.ORACLE_VARCHAR)
								.append("(100)");
					}
					if (iCount != iSize) {
						strCreateQuery.append(',');
				...
				strCreateQuery.append(')');
				lstFinalQueries.add(strCreateQuery.toString());
			}
			managerentity.getTransaction().begin();
			for (String strQuery : lstFinalQueries) {
				managerentity.createNativeQuery(strQuery).executeUpdate();
			}
			managerentity.getTransaction().commit();
		}
		logger.info(strClassName + strMethodName + " return saveTablesDetails method");
		return strResponse;
	}

	@Override
	public void doDumpTableAliasValues(Map<String, List<String>> mapExcelValues,String userid,
			EntityManager entityManager){

		String strMethodName = " [ doDumpManualDictionaryValues() ] ";
		logger.info(strClassName + strMethodName + " inside doDumpManualDictionaryValues method");
		if ( mapExcelValues != null && !mapExcelValues.isEmpty()) {
			StringBuffer strBufInsertQuery = new StringBuffer("INSERT INTO tdg_table_alias(alias_id,tabname,alias_name,SCHEMA_NAME,userid)Values(?,?,?,?,?)");
			Long no = (Long) entityManager.createQuery(
...
/*			StringBuffer strSuffix = new StringBuffer();
			...
			/* Going for get jdbc template */
			final String strFinalQuery = strBufInsertQuery.toString();
			int iMaxSize = mapExcelValues.get("TABNAME").size();
			if(mapExcelValues.get("ALIAS_NAME").size()>iMaxSize)
				iMaxSize=mapExcelValues.get("ALIAS_NAME").size();
			final int maxSize = iMaxSize;
			...
			List<List<String>> lstString = new ArrayList<List<String>>();
			lstString.add(listInt);
			lstString.add(mapExcelValues.get("TABNAME"));
			lstString.add(mapExcelValues.get("ALIAS_NAME"));
			lstString.add(mapExcelValues.get("SCHEMA_NAME"));
			lstString.add(listuserId);
			final List<List<String>> lstListString = lstString;
			entityManager.unwrap(Session.class).doWork(new Work(){
				@Override
				public void execute(Connection connection) throws SQLException{
	...
	@Override
	public TdgSchemaDO fetchSchemaDetailsByName(String dictionaryName, EntityManager managerEntity){

		String strMethodName = " [ fetchSchemaDetailsByName() ] ";
		logger.info(strClassName + strMethodName + " inside fetchSchemaDetailsByName method");
		//List<TdgSchemaDO> listTdgSchemaDao = new ArrayList<TdgSchemaDO>();
		JdbcTemplate template = null;
		TdgSchemaDO tempTdgSchemaDO = new TdgSchemaDO();
		try {
			if (managerEntity != null) {
				TdgSchemaDO tdgSchemaDO = managerEntity.createNamedQuery("TdgSchemaDO.findBySchemaName",
						TdgSchemaDO.class).setParameter("schemaname", dictionaryName).getSingleResult();
				//for (TdgSchemaDO tdgSchemaDO : tempTdgSchemaDao) {
					template = getTemplate(tdgSchemaDO.getUrl(), tdgSchemaDO.getUsername(),
							tdgSchemaDO.getPassword());
					//TdgSchemaDO tempTdgSchemaDO = new TdgSchemaDO();
				...
				//}
				logger.info(strClassName + strMethodName
						+ " got the schema details ");
			}
		} finally {
			cleanupDataSource(template);
		}
		logger.info(strClassName + strMethodName + " return from fetchSchemaDetailsByName method");
		return tempTdgSchemaDO;
