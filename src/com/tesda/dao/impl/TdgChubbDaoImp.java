...
@Component("tdgChubbCustomDao")
public class TdgChubbDaoImp implements TdgCustomDao{
		...
		if (!listURLs.isEmpty() && !listUserNames.isEmpty() && !listPasswords.isEmpty() ) {
			...
			if (!mapConditionValues.isEmpty()) {
				...
				for (Map.Entry<String, List<String>> mapentry : mapConditionValues.entrySet()) {
					if(!TdgCentralConstant.TDG_MNLDT_SEQ.equalsIgnoreCase(mapentry.getKey())){
				...
				if (null != mapValue.getValue() && !"".equals(mapValue.getValue())
					...
					if (mapValue.getKey().contains("#")) {
						...
						if (mapValue.getValue().toString().contains("#")) {
							...
							mapConditionsPassed.put(mapValue.getKey(), lst);
						}else if(mapValue.getValue().toString().equalsIgnoreCase("TDG_SSN")){
							mapConditionsPassed.put(mapValue.getKey(), TdgAutoIncrementUtil.generateSSN(testDataGenenarteDTO.getGenerateRecordsCount()));
						}else {
				...
				for (Map.Entry<String, List<String>> mapValue : mapConditions.entrySet()) {
					...
					if (null != mapValue.getValue() && mapValue.getValue().get(0) != null && !"".equals(mapValue.getValue().get(0))
						...
						if (mapValue.getKey().contains("#")) {
							String strArray[] = mapValue.getKey().split("#");
							for (int ii = 0; ii < strArray.length; ii++) {
								if (mapValue.getValue().get(0).equalsIgnoreCase("TDG_SSN")) {
									
									mapConditionsPassed.put(mapValue.getKey(), TdgAutoIncrementUtil.generateSSN(testDataGenenarteDTO.getGenerateRecordsCount()));
								}else if (mapValue.getValue().get(0).contains("#")) {
	...
	public Map<String, Vector<String>> generateQueriesAndDumping(JdbcTemplate jdbcTemplate,
		...
		//add for chubb account,basically storing the pk values in heap
		@SuppressWarnings("rawtypes")
		Map<String,Vector> mapPkValsRef = new WeakHashMap<String,Vector>();
		/*List<String> listPkValsWithOne = new ArrayList<String>();
		List<String> listPkValsWithTwo = new ArrayList<String>();
		List<String> listPkValsWithThree = new ArrayList<String>();*/
		Map<Object,List<Object>> mapBusinesschubb = new WeakHashMap<Object, List<Object>>();
		Map<Object,String> mapMailCombine = new WeakHashMap<Object, String>();
		Map<Object,Object> mapPartyNmWthParty = new WeakHashMap<Object, Object>();
		//end values
		...
		Map<String, Set<String>> mapCompositPKs = new WeakHashMap<String, Set<String>>();
			...
			for (List<Object> listChildTabs : listResultTables) {
				...
				for (Object obj : listChildTabs) {
					...
					if (!mapPks.containsKey(strTableName) && !listMasterTabs.contains(strTableName)
						...
						StringBuffer strInsertQuery = new StringBuffer("INSERT INTO "+ strTableName + "(");
						...
						for (Map<String, Object> mapResult : listPKValues) {
							...
							if (!listUkCheck.contains(String.valueOf(mapResult.get("COLUMN_NAME")))) {
								...
								if (i > 0) {*/
									...
									if(listPKValues.size()>1){
										hasSet.add(String.valueOf(mapResult.get("COLUMN_NAME")));
									}
								//}
							} /*else {
						 ...
						 */
						if(!hasSet.isEmpty())
						mapCompositPKs.put(strTableName, hasSet);
						...
						for (Map<String, Object> mapResult : listPKValues) {
							...
							if(!mapPkValsRef.containsKey(String.valueOf(mapResult.get("COLUMN_NAME"))) ){
								mapNNkValues.put(
										String.valueOf(mapResult.get("COLUMN_NAME")),
...
*/									lstIdentifiedcolumns.add(String.valueOf(mapResult.get("COLUMN_NAME")));
						...
						if(testDataGenenarteDTO.getGenerationType().equalsIgnoreCase("DB")){
							Set<String> computedColumns = tdgTemplateDao.getComputedColumns(jdbcTemplate,strTableName);
							for(Map.Entry<String, Integer> mapFinalCols: mapTabColumnSequences.entrySet()){
								...
								if(!computedColumns.contains(mapFinalCols.getKey())){
					...
					for (String tbName : listFinalSequence) {
						...
						if (!mapPks.containsKey(tbName) && !listMasterTabs.contains(tbName)
							...
							if (lstTabsNames.contains(tbName)) {
								/*if("PARTY_NM".equalsIgnoreCase(tbName)||"PARTY_INDVDL_NM".equalsIgnoreCase(tbName))
									lRecordsCount = iRequiredRecordCount*3;
								else*/
							...
							if (mapNNkNames != null && !mapNNkNames.isEmpty()) {
								...
								for (Map.Entry<String, String> mapEntry : mapNNkNames.entrySet()) {
									String strValue = mapEntry.getValue();
									String strKey = mapEntry.getKey();
									//added for chubb same value finding
									if(mapPkValsRef.containsKey(strKey)){
										mpNNkValues.put(strKey, mapPkValsRef.get(strKey));
										lstGeneratedValues.add(strKey);
										//continue;
									}else if(mapPkValsRef.containsKey(strKey+"ESS")){
										mpNNkValues.put(strKey, mapPkValsRef.get(strKey+"ESS"));
										lstGeneratedValues.add(strKey);
									}
									List<String> listGenValues = new ArrayList<String>();
									if(!lstGeneratedValues.contains(mapEntry.getKey())){
							
									if (mapDataDictionaryVals.containsKey(mapEntry.getKey()) || mapDataDictionaryVals.containsKey(tbName+TdgCentralConstant.TDG_DOT+mapEntry.getKey())) {
										if (isDataConditional) {
											listGenValues.addAll(mapDataDictionaryVals.get(mapEntry
													.getKey()));
										} else {
									...
									} else if ("DATE".equals(strValue) || "TIMESTAMP".equals(strValue)) {
											DateFormat dateFormat = new SimpleDateFormat(
													StringUtils.isEmpty(testDataGenenarteDTO
															.getDateFormate()) ? "dd/MMM/yyyy hh:mm:ss"
															: testDataGenenarteDTO.getDateFormate());
							...
							if (mapConditions.containsKey(mapConditionWithPKs.get(tbName))) {
								...
								if (mapFinalFilter != null && !mapFinalFilter.isEmpty()) {
									...
									//going to check chubb refernces because no relationship maintain
									
									if(mapPkValsRef.containsKey(vct.get(0))){
										vctValues.addAll(mapPkValsRef.get(vct.get(0)));
										//continue;
									}else{
									//end of validation
									...
									if(strVal.matches(regexForNumbers))
										strDbSeq = TdgCentralConstant.getSpecificDBQuery(tdgTemplateDao.getDbType(),
												TdgCentralConstant.PRIMARY_KEY_FINAL, vct);
									if("PARTY_NM".equalsIgnoreCase(tbName) || "PARTY_INDVDL_NM".equalsIgnoreCase(tbName))
										vctValues.addAll(TdgAutoIncrementUtil.generateAutoIncrementValues(tdgTemplateDao.getMaxValue(jdbcTemplate,strDbSeq), (long)((lRecordsCount*3)+1)));
									else
									vctValues.addAll(TdgAutoIncrementUtil.generateAutoIncrementValues(tdgTemplateDao.getMaxValue(jdbcTemplate,strDbSeq), (long)lRecordsCount+1));
									vctValues.remove(0);
									}
							...
							for(Map.Entry<String, Object> mapEntry : mapTempConditions.entrySet()){
								if(!mapPkVals.containsKey(mapEntry.getKey()) && String.valueOf(mapEntry.getValue()).contains(TdgCentralConstant.TDG_DB_SEQ) && !mapUkNames.containsKey(mapEntry.getKey()) && !mapNNkNames.containsKey(mapEntry.getKey()) && mapTabsWithTables.get(tbName).contains(mapEntry.getKey())){
									...
									if(StringUtils.isEmpty(strInputValue)){									
									Vector<String> vt = new Vector<String>();
									vt.add(mapEntry.getKey());
									vt.add(tbName);
									String strDbSeq = TdgCentralConstant.getSpecificDBQuery(tdgTemplateDao.getDbType(),
									...
									}else if("_VCHR".equalsIgnoreCase(strInputValue)){
										//this blocked added for chubb purpose
										Vector<String> vt = new Vector<String>();
										vt.add(tbName);
										vt.add(mapEntry.getKey());										
										String strDbSeq = TdgCentralConstant.getSpecificDBQuery(tdgTemplateDao.getDbType(),
												TdgCentralConstant.CHUBB_UNQ_DEF, vt);
										vctVal.addAll(TdgAutoIncrementUtil.generateAutoIncrementValues(tdgTemplateDao.getMaxValue(jdbcTemplate,strDbSeq), (long)lRecordsCount+1));
										vctVal.remove(0);
									}else{
							...
							//add below values for chubb
							mapPkValsRef.putAll(mapPkVals);
							
							//going to check sequece of manual dictionary values
							Set<String> seqCols = new HashSet<String>();
							if(mapConditions.containsKey(TdgCentralConstant.TDG_MNLDT_SEQ)){
								String[] splitCols = mapConditions.get(TdgCentralConstant.TDG_MNLDT_SEQ).toString().split(",");
								for(String strCol : splitCols){
									seqCols.add(strCol);
								}
							}
							//end
                           ...
                           // List<Object[]> listParameters = null;
                            for(long i2 = 0;i2 < iLoop;i2++){
                                ...
                                Map<String,Integer> mapRev = TdgCentralConstant.reverse(mapPositions);
        								...
        								if (listRelations != null) {
        									...
        									for (int k = 0; k < mapPositions.size(); k++) {
        										...
        										if(mapPkVals.containsKey(mapPositions.get(k+1)) && !mapPkVals.get(mapPositions.get(k+1)).isEmpty()){
        											if("PARTY_NM".equalsIgnoreCase(tbName) || "PARTY_INDVDL_NM".equalsIgnoreCase(tbName))
        													obj[k] = vctValues.get(iElement*3);
        											else
        											...
        											if(mpNNkValues.get(mapPositions.get(k + 1)) != null && mpNNkValues.get(mapPositions.get(k + 1)).size() ==1){
        												...
        												//if(mpNNkValues != null && mpNNkValues.size() != lRecordsCount){ //code was commented based on NNvalues size check
        												if(mpNNkValues != null && mpNNkValues.get(mapPositions.get(k + 1)).size() != lRecordsCount){
        													//added for chubb purpose sequence values
        													if(seqCols.contains(mapPositions.get(k + 1))){
        														if(iDictionaryPickedRowId == 0)
        															iDictionaryPickedRowId = (int)(Math.random() * (mpNNkValues.get(mapPositions.get(k + 1)).size()));
        														obj[k] = mpNNkValues.get(mapPositions.get(k + 1)).get(iDictionaryPickedRowId);
        													}else
        									...
        									for (int k = 0; k < mapPositions.size(); k++) {
        										...
        										if("$TDG_MAIL_CONFIG$".equalsIgnoreCase(strFindValue)){
        											if(mapMailCombine.containsKey(obj[mapRev.get("G_PARTY")-1]))
        												obj[mapRev.get("T_EMAIL_ADDR")-1]=mapMailCombine.get(obj[mapRev.get("G_PARTY")-1]);
        										}
        										else if(strFindValue.contains("$")){
        											StringBuffer strBuffer = new StringBuffer();
        											...
        											for(int iSize=0;iSize<splitsvalues.length;iSize++){
        												...
        												if(mapPositions.containsValue(strCondition.replaceAll("TDG_UPPER", ""))){
        													//for (Map.Entry<Integer, String> mapEntry : mapPositions.entrySet()) {
        														if(mapRev.containsKey(strCondition.replaceAll("TDG_UPPER", ""))){
        															String strTemp = obj[mapRev.get(strCondition.replaceAll("TDG_UPPER", ""))-1].toString();
        															//assumention as started with constant TDG_UPPER
        															if(strCondition.contains("TDG_UPPER"))
        																strBuffer.append("TDG_UPPER");
        													...
        													//}
        												}else if(mapPositions.containsValue(strCondition.replaceAll("TDG_SOUNDEX", ""))){
        													//for (Map.Entry<Integer, String> mapEntry : mapPositions.entrySet()) {
    														if(mapRev.containsKey(strCondition.replaceAll("TDG_SOUNDEX", ""))){
    															String strTemp = obj[mapRev.get(strCondition.replaceAll("TDG_SOUNDEX", ""))-1].toString();
    															//assumention as started with constant TDG_UPPER
    															//if(strCondition.contains("TDG_SOUNDEX"))
    																strBuffer.append("TDG_SOUNDEX");
    															strBuffer.append(splitsvalues[iSize].contains("{") ? strTemp.substring(Integer.parseInt(splitsvalues[iSize].substring(splitsvalues[iSize].indexOf("{")+1,splitsvalues[iSize].indexOf(",")) != null ? splitsvalues[iSize].substring(splitsvalues[iSize].indexOf("{")+1,splitsvalues[iSize].indexOf(",")).trim() : "0"),Integer.parseInt(splitsvalues[iSize].substring(splitsvalues[iSize].indexOf(",")+1,splitsvalues[iSize].indexOf("}")) != null ? splitsvalues[iSize].substring(splitsvalues[iSize].indexOf(",")+1,splitsvalues[iSize].indexOf("}")).trim() : "0")) : strTemp);
    															//need to verify the break statement of map position values
    															//break;
    														}
        										...
        										obj[k] = strBuffer.toString();
        										if(strBuffer.toString().startsWith("TDG_UPPER"))
        										obj[k] = strBuffer.toString().replaceAll("TDG_UPPER", "").toUpperCase();
        										if(strBuffer.toString().startsWith("TDG_SOUNDEX"));
            										//obj[k] = soundex.soundex(strBuffer.toString().replaceAll("TDG_SOUNDEX", ""));        										
        									...
        									//commented below object for chubb customization 
        									//listParameters.add(obj);
        									
        									//going to customize PARTY_NM ,PARTY_INDVDL_NM values
        									
        									if("PARTY_NM".equalsIgnoreCase(tbName))
        									{
        										//Map<String,Integer> mapRev = TdgCentralConstant.reverse(mapPositions);
        										int iPks = iElement*3;
        										List<Object> lstSequence = new LinkedList<Object>();
        										for(int icust=0;icust<3;icust++){
        											Object[] objct = new Object[obj.length];//listParameters.get(iRecord);
        											objct  = obj.clone();
        											if(icust==0){
        												objct[mapRev.get("B_PRIM")-1]="1";
        												objct[mapRev.get("B_DFLT")-1]="1";
        												//listPkValsWithOne.add(String.valueOf(objct[mapRev.get("G_PARTY_NM")]));
        												iPks++;
        											}else{
        												objct[mapRev.get("G_PARTY_NM")-1]=vctValues.get(iPks);
        												objct[mapRev.get("B_PRIM")-1]="0";
        												objct[mapRev.get("B_DFLT")-1]="0";
        												iPks++;        												
        											}
        											lstSequence.add(objct[mapRev.get("G_PARTY_NM")-1]);
        											/*mapBusinesschubb.put(objct[mapRev.get("G_PARTY")], lstSequence);
        											mapPartyNmWthParty.put(objct[mapRev.get("G_PARTY_NM")], objct[mapRev.get("G_PARTY")]);*/
        											mapPartyNmWthParty.put(objct[mapRev.get("G_PARTY_NM")-1], objct[mapRev.get("G_PARTY")-1]);
        											listParameters.add(objct);
        										}
        										mapBusinesschubb.put(obj[mapRev.get("G_PARTY")-1], lstSequence);
    											//mapPartyNmWthParty.put(objct[mapRev.get("G_PARTY_NM")], objct[mapRev.get("G_PARTY")]);
        										
        									}else if("PARTY_INDVDL_NM".equalsIgnoreCase(tbName))
        									{
        										//Map<String,Integer> mapRev = TdgCentralConstant.reverse(mapPositions);
        										//int iPks = iElement;
        										//Object[] objct = obj;//listParameters.get(iRecord);
    											List<Object> lstSequence = mapBusinesschubb.get(mapPartyNmWthParty.get(obj[mapRev.get("G_PARTY_NM")-1]));
    											mapMailCombine.put(mapPartyNmWthParty.get(obj[mapRev.get("G_PARTY_NM")-1]), String.valueOf(obj[mapRev.get("T_FRST_NM")-1])+"."+String.valueOf(obj[mapRev.get("T_LAST_NM")-1])+TdgCentralConstant.TDG_MAIL_TYPES.get(new Random().nextInt(3)));
        										//for(int icust=0;icust<3;icust++){
        											//Object[] objct = obj;
        											for(int ichild=0;ichild<lstSequence.size();ichild++){
        												Object[] objct = new Object[obj.length];//listParameters.get(iRecord);
            											objct  = obj.clone();
        												if(ichild==0){
        													objct[mapRev.get("C_NM_TYP")-1]="01";
        													
        												}else{
        													if(ichild==1){
            													objct[mapRev.get("C_NM_TYP")-1]="06";
            												}else
            													objct[mapRev.get("C_NM_TYP")-1]="07";
        												}
        												objct[mapRev.get("G_PARTY_NM")-1]=lstSequence.get(ichild);
        												listParameters.add(objct);
        											}
        											
        											/*if(icust==0){
        												objct[mapRev.get("C_NM_TYP")]="01";
        												iPks++;
        											}else{
        												objct[mapRev.get("G_PARTY_NM")]=vctValues.get(iPks);
        												if(icust ==2)
        												objct[mapRev.get("C_NM_TYP")]="06";
        												else
        												objct[mapRev.get("C_NM_TYP")]="07";
        											}*/
        											//listParameters.add(objct);
        										//}
