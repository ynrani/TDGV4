  ...
  $(document).ready(function() {
	  $('#errors').html('');
		...
		$('#connection').change(function() {
			$('#manualDictionary').remove();
			$('#uploadManualDictionary').append('<input type="hidden" id="manualDictionary" name="manualDictionary" value="'+$(this).val()+'" />');
			});
	})
...
<body>
     <form:form id="uploadManualDictionary" action="${pageContext.request.contextPath}/uploadManualDictionary" modelAttribute="tdgDictionaryDTO" enctype="multipart/form-data">
            ...
            <table style="text-align: center; padding: 1%; width: 80%;">               
                <tr>
                    <td>
						...
						<tr>
							<td class="flied-title"> OR 
							<select id="connection" class="form-control" >
							<option value=""></option>
		<c:forEach items="${tdgDictionaryDTO.listManualDictionaries}" var="listManualDictionaries" varStatus="status">
		<option value="${listManualDictionaries}">${listManualDictionaries}</option>
		</c:forEach>
		</select> 
							</td>
						</tr>
